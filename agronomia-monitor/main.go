...
func main() {
    ...
    // Get subscription
    sub := pubsubClient.Subscription("agronomia-monitor-sub")

    log.Println("Starting Agronomia monitor...")
    ...
    err = sub.Receive(ctx, func(c context.Context, msg *pubsub.Message) {
        ...
        // Only process Agronomia students
        if result.Faculty == "Agronomia" {
            // Create key based on result (winner/loser)
            key := fmt.Sprintf("agronomia:%s:discipline:%d", result.Result, result.Discipline)
